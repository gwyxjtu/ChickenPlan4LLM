# 结构化语言 -> 描述性json  #3@FU39，知识输入好像差个案例，但是好像不重要
info_prompt_template = [
    """
你是一个优化专家团队中的数学建模专家。团队的目标是解决一个能源系统设备容量规划的最优化问题。\
你的责任是读取、分析和提取用户输入中用于能源系统建模的信息，来构建并输出一个描述模型背景、约束条件、目标函数、参数和优化变量的JSON文件。
""",
    """
现在请根据由```分隔的示例，学习：如何分析**用户输入**的内容，并提取其中的关键信息；\
在**相关知识**中找到对应设备的建模信息和对应地理区域的光伏出力和供电、冷、热价格等信息；\
从而构建遵守示例中**输出**格式的JSON。
示例：```
**用户输入**：
{example_user_input}
**相关知识**：
...
请注意：
- 只生成JSON文件，不要生成其他文本，并且对于不要省略任何信息，包括从相关知识中读取的信息。
- 使用"```json"和"```"来包围JSON文件。
- 对于约束条件中的每一条，只生成一个约束条件字符串，不要包含多个约束条件。
- 重要：确保输出的JSON文件包含根据用户输入从相关知识中检索得到的信息，确保用户输入中每个设备都得到建模；\
输出的JSON需要遵守示例中**输出**格式。
...
你的责任是根据"用户输入的结构化描述与需求"和"从用户输入中提取对应优化问题的模型背景、约束条件、目标函数、参数和优化变量等信息"，生成该最优化问题所需参数的json文件，其中需包含各参数的值，且值需符合用户对参数值的需求。
""",
    """
...
code_prompt_cot_template = [
    """
你是一个优化专家团队中的专业程序员。团队的目标是解决一个能源系统设备容量规划的最优化问题。\
你的责任是根据"优化问题的模型背景、约束条件、目标函数、参数和优化变量等描述信息"和"该优化问题所需参数"，编写{solver}代码。
